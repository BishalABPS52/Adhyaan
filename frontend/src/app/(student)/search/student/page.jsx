...
export default function StudentSearchSection() {
  ...
  const fetchStudyBooks = async () => {
    ...
    try {
      const response = await fetch(`${baseUrl}/books/?book_type=academic&limit=100`);
      if (response.ok) {
  ...
  const boardOptions = ['All', 'TU', 'KU', 'PU Pokhara', 'PU Purbanchal', 'NEB', 'CBSE', 'CTEVT', 'Others'];
  const yearOptions = ['All', '1', '2', '3', '4'];
  const partOptions = ['All', 'I', 'II'];
  const semOptions = ['All', '1', '2', '3', '4', '5', '6', '7', '8'];
  ...
  const filteredBooks = studyBooks.filter(book => {
    ...
    const matchesBoard = selectedBoard === 'all' || bookBoard === selectedBoard.toLowerCase();
    ...
    return matchesSearch && matchesBoard && matchesYear && matchesPart && matchesSem;
  });
    ...
    <div className={styles.container}>
      <div className="container">
        <div className={styles.header}>
          ...
          <p className={styles.pageSubtitle}>Explore academic books based on board, course and curriculum</p>
        </div>
        ...
        <Card className={styles.searchCard}>
          ...
          <div className={styles.filters}>
            ...
            <div className={styles.filterGroup}>
              ...
              >
                {semOptions.map(sem => (
                  <option key={sem} value={sem.toLowerCase()}>{sem}</option>
                ))}
              </select>
            </div>
          </div>
